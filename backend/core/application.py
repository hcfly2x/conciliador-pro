    ...
    for row in rows:
        try:
            date_normal = bool(
                ...
                and abs(
                    (
                        dt.date.fromisoformat(tx_date) - dt.date.fromisoformat(row[1])
                    ).days
                )
                <= 2
            )
        ...
        if installment_match:
            try:
                ...
                date_options = [
                    value for value in (tx_date, installment_first_date) if value
                ]
                ...
                installment_date_normal = (
                    abs(
                        (
                            dt.date.fromisoformat(installment_comparison_date)
                            - history_date_value
                        ).days
                    )
                    <= 7
...
@transactions_bp.route("/api/v1/transactions/<tx_id>/unlink-history", methods=["PATCH"])
...
@history_links_bp.route("/api/v1/transactions/<tx_id>/history-link", methods=["POST"])
@allow_collab_write
...
def confirm_history_link_in_batch(conn, tx_id: str) -> tuple[dict[str, Any], str]:
    ...
    validation_error, _validation_status = validate_classification_selection(
        conn, history[0], history[1], row[4]
    )
    if validation_error:
        return {}, str(
...
def strict_history_match_for_batch(
    ...
    for history in history_index.get(standard_key, []):
        ...
        if history_id == rejected_id:
            continue
        similarity = round(
            description_similarity(tx_description, history[2] or "") * 100, 1
        )
        if similarity <= 95.0:
            continue
        candidate = {
...
@history_links_bp.route("/api/v1/transactions/history-links/batch", methods=["POST"])
@allow_collab_write
    ...
    with db_connect() as conn:
        ...
        for position, tx_id in enumerate(ids, start=1):
            ...
            if (
                ...
                or bool(current_state[0])
                or bool(current_state[1])
            ):
            ...
            if not identity:
                manual_identity = find_identity_match(
                    conn,
                    {
                        "date": row[1],
                        "description": row[2],
...
@suggestions_bp.route("/api/v1/transactions/suggestions/jobs", methods=["POST"])
@allow_collab_write
...
@suggestions_bp.route("/api/v1/suggestion-jobs/<job_id>")
...
@suggestions_bp.route("/api/v1/suggestion-jobs-active")
...
@suggestions_bp.route("/api/v1/transactions/suggestions/summary")
...
@suggestions_bp.route("/api/v1/transactions/suggestions/evaluation")
...
@suggestions_bp.route(
    "/api/v1/transactions/<tx_id>/suggestions/dismiss", methods=["POST"]
)
@allow_collab_write
def dismiss_transaction_suggestions(tx_id: str):
    with db_connect() as conn:
        ...
        record_audit(
            current_user(), "dismiss_suggestions", "transaction", tx_id, conn=conn
        )
    return jsonify({"id": tx_id, "dismissed": True, "ok": True})


@suggestions_bp.route("/api/v1/transactions/suggestions/batch", methods=["POST"])
@allow_collab_write
...
@suggestions_bp.route("/api/v1/transactions/<tx_id>/suggestions")
...
@history_links_bp.route(
    "/api/v1/transactions/recalculate-probabilities", methods=["POST"]
)
@history_links_bp.route(
    "/api/v1/transactions/recalculate-history-links", methods=["POST"]
)
...
@history_links_bp.route("/api/v1/recalculation-jobs/<job_id>")
...
@history_links_bp.route("/api/v1/history")
def classification_history_list():
    ...
    search = (request.args.get("search") or "").strip()
    if search:
        s_norm = f"%{escape_like(norm_text(search))}%"
        s_raw = f"%{escape_like(search)}%"
        where.append(
            "("
        ...
        s_lower = f"%{escape_like(search.lower())}%"
